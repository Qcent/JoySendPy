...
def print_xbox_report(report: XBOX_REPORT) -> None:
    """
    Prints the state of an Xbox controller.

    Args:
        report (XBOX_REPORT): an XINPUT_GAMEPAD-compatible report structure

    Returns:
        None

    Raises:
        TypeError: if `report` is not an instance of `XBOX_REPORT`
    """

    if not isinstance(report, XBOX_REPORT):
        raise TypeError("report must be an instance of XBOX_REPORT")
