...
AXIS_INPUT_DEADZONE = 3600
RUMBLE_CONVERSION = .00392


class PyGameButtonMapping_old:
    class ButtonMap:
...
class PyGameButtonMapping:
    ...
    def __init__(self):
        ...
        # Stick directions (added from C++ code)
        self.LEFT_STICK_LEFT = self.ButtonMap()
        self.LEFT_STICK_RIGHT = self.ButtonMap()
        self.LEFT_STICK_UP = self.ButtonMap()
        self.LEFT_STICK_DOWN = self.ButtonMap()
        self.RIGHT_STICK_LEFT = self.ButtonMap()
        self.RIGHT_STICK_RIGHT = self.ButtonMap()
        self.RIGHT_STICK_UP = self.ButtonMap()
        self.RIGHT_STICK_DOWN = self.ButtonMap()

    # Display method to show all mappings
    def display_button_maps(self):
        button_maps = [attr for attr in dir(self) if isinstance(getattr(self, attr), self.ButtonMap)]
        for button_map in button_maps:
            print(
                f"Name: {button_map}, Input Type: {getattr(self, button_map).input_type}, "
                f"Type Index: {getattr(self, button_map).index}, Value: {getattr(self, button_map).value}")

    # Save all mappings to a file
    def save_button_maps(self, filename):
        button_list = []
        button_maps = [attr for attr in dir(self) if isinstance(getattr(self, attr), self.ButtonMap)]
        for button_map in button_maps:
    ...
    # Get button names that have been set
    def get_set_button_names(self):
        return [attr for attr in dir(self) if
                isinstance(getattr(self, attr), self.ButtonMap) and getattr(self, attr).input_type is not None]

    # Get button names that have not been set
    def get_unset_button_names(self):
        return [attr for attr in dir(self) if
                isinstance(getattr(self, attr), self.ButtonMap) and getattr(self, attr).input_type is None]

    # Get stick button names
    def get_all_stick_button_names(self):
        button_names = self.get_all_button_names()
        return [name for name in button_names if 'stick' in name.lower()]

    # Get trigger button names
    def get_all_trigger_button_names(self):
        button_names = self.get_all_button_names()
        return [name for name in button_names if 'trigger' in name.lower()]

    # Get thumb button names
    def get_all_thumb_button_names(self):
        button_names = self.get_all_button_names()
        return [name for name in button_names if 'thumb' in name.lower()]

    # Get shoulder button names
    def get_all_shoulder_button_names(self):
        button_names = self.get_all_button_names()
        return [name for name in button_names if 'shoulder' in name.lower()]

    # Get dpad button names
    def get_all_dpad_button_names(self):
        button_names = self.get_all_button_names()
        return [name for name in button_names if 'dpad' in name.lower()]

    # Get generic button names
    def get_all_generic_button_names(self):
        button_names = self.get_all_button_names()
        generic_names = [name for name in button_names if not ('dpad' in name.lower() or 'shoulder' in name.lower()
                                                               or 'thumb' in name.lower() or 'trigger' in name.lower()
...
def get_xbox_report_from_hidmap(gamepad, report_size, buttons, input_lists: ([[str]]), xbox_report: XBOX_REPORT):
    ...
    for input_name in stick_list:
        ...
        if input_name == 'LEFT_STICK_X':
            xbox_report.sThumbLX = byte_to_c_short(button_value)
        if input_name == 'LEFT_STICK_Y':
            xbox_report.sThumbLY = byte_to_c_short(button_value)
        if input_name == 'RIGHT_STICK_X':
            xbox_report.sThumbRX = byte_to_c_short(button_value)
        if input_name == 'RIGHT_STICK_Y':
            xbox_report.sThumbRY = byte_to_c_short(button_value)
    ...
    for input_name in trigger_list:
        ...
        if input_name == 'LEFT_TRIGGER':
            xbox_report.bLeftTrigger = byte_to_c_byte(button_value)
        if input_name == 'RIGHT_TRIGGER':
            xbox_report.bRightTrigger = byte_to_c_byte(button_value)
...
def get_xbox_input_from_bytearray(name: str, comparison: str,
    ...
    if get_input_type(name) == 2:  # Stick by button
        return bits * 255

    # Compare bits to comparison value
    if bits == int(comparison, 2):
        if get_input_type(name) == 3:  # Trigger by button
            return 255
...
def wait_for_no_hid_gamepad_input(gamepad, ignore_indices, stick_indices):
    ...
    while awaiting_silence:
        ...
        results = get_diff_in_bytearrays(avg_baseline, update, ignore_indices)
...
def receive_single_hid_input_map(gamepad, ignore_indices, stick_indices):
    ...
    while True:
        ...
        for byte in results:
            ...
            bits = get_bits_different(byte['first_value'], byte['second_value'])
            ...
            if len(results) == 1 and len(bits) == 1 and byte['index'] not in stick_indices:
                # Single bit button
                offset_bit = bits[0]
                value = f'{get_bit_value(update, offset_byte, offset_bit)}'
...
def set_hid_mapping(gamepad, buttons: HIDButtonMapping, input_list: [str]):
    ...
    first_baseline, avg_baseline, median_baseline, mode_baseline, range_baseline = \
        get_hid_data_stream_reports(gamepad)
    report_size = len(first_baseline)

    # Rule out non-user input fields
    ignore_indices = find_values_in(range_baseline, '>0')  # values that changed at all during the sample
    ignore_indices += find_values_in(avg_baseline, '>133')  # average values higher than expected neutral sticks

    # Intuit likely stick indices
    stick_step1 = find_values_in(avg_baseline, '~127/12')  # expected neutral stick average within 12
    stick_step2 = find_values_in(range_baseline, '~0/9')  # expected neutral stick range within 9
    ...
    for input_name in input_list:
        input_type = get_input_type(input_name)
...
def get_xbox_report_from_pymap(pygame, gamepad, buttons, input_list: [str], xbox_report: XBOX_REPORT):
    ...
    xbox_report.sThumbLX = 0
    xbox_report.sThumbLY = 0
    xbox_report.sThumbRX = 0
    xbox_report.sThumbRY = 0
    xbox_report.bLeftTrigger = 0
    xbox_report.bRightTrigger = 0

    # Receive new input events
    pygame.event.pump()  # Efficiently update the event queue

    for input_name in input_list:
        typ = getattr(buttons, input_name).input_type
        idx = getattr(buttons, input_name).index
        val = getattr(buttons, input_name).value

        # Hat Value
        if typ == 4:
            button_value = gamepad.get_hat(idx)
            is_pressed = (val & button_value) != 0  # Bitwise AND compare
            if is_pressed:
                if input_name == 'LEFT_STICK_LEFT':
                    xbox_report.sThumbLX = -32767
                elif input_name == 'LEFT_STICK_RIGHT':
                    xbox_report.sThumbLX = 32767
                elif input_name == 'LEFT_STICK_UP':
                    xbox_report.sThumbLY = 32767
                elif input_name == 'LEFT_STICK_DOWN':
                    xbox_report.sThumbLY = -32767
                elif input_name == 'RIGHT_STICK_LEFT':
                    xbox_report.sThumbRX = -32767
                elif input_name == 'RIGHT_STICK_RIGHT':
                    xbox_report.sThumbRX = 32767
                elif input_name == 'RIGHT_STICK_UP':
                    xbox_report.sThumbRY = 32767
                elif input_name == 'RIGHT_STICK_DOWN':
                    xbox_report.sThumbRY = -32767
                elif input_name == 'LEFT_TRIGGER':
                    xbox_report.bLeftTrigger = 255
                elif input_name == 'RIGHT_TRIGGER':
                    xbox_report.bRightTrigger = 255
                else:
        ...
        # Axis Value
            ...
            converted_val = float_to_c_short(button_value)
            same_sign = have_same_sign(button_value, val)
            abs_val = max(abs(int.from_bytes(converted_val, byteorder='little', signed=True)), 0)
            if abs_val and same_sign:
                if input_name == 'LEFT_STICK_LEFT':
                    xbox_report.sThumbLX = -abs_val
                elif input_name == 'LEFT_STICK_RIGHT':
                    xbox_report.sThumbLX = abs_val
                elif input_name == 'LEFT_STICK_UP':
                    xbox_report.sThumbLY = abs_val
                elif input_name == 'LEFT_STICK_DOWN':
                    xbox_report.sThumbLY = -abs_val
                elif input_name == 'RIGHT_STICK_LEFT':
                    xbox_report.sThumbRX = -abs_val
                elif input_name == 'RIGHT_STICK_RIGHT':
                    xbox_report.sThumbRX = abs_val
                elif input_name == 'RIGHT_STICK_UP':
                    xbox_report.sThumbRY = abs_val
                elif input_name == 'RIGHT_STICK_DOWN':
                    xbox_report.sThumbRY = -abs_val
                elif input_name == 'LEFT_TRIGGER':
                    xbox_report.bLeftTrigger = int(abs_val * 255)
                elif input_name == 'RIGHT_TRIGGER':
                    xbox_report.bRightTrigger = int(abs_val * 255)
                else:
                    if abs_val > AXIS_INPUT_DEADZONE:
                        xbox_report.wButtons += getattr(XBOX_BUTTON, 'XBOX_' + input_name)
...
def get_xbox_report_from_pymap_old(pygame, gamepad, buttons, input_list: [str], xbox_report: XBOX_REPORT):
    # Reset button values
    xbox_report.wButtons = 0
    xbox_report.sThumbLX = 0
    xbox_report.sThumbLY = 0
    ...
    for input_name in input_list:
        ...
        if typ == 4:
            ...
            if val == button_value:
                # Assign value to the proper XBOX report field
                if input_name == 'LEFT_STICK_X':
                    xbox_report.sThumbLX = float_to_c_short(button_value)
                elif input_name == 'LEFT_STICK_Y':
                    xbox_report.sThumbLY = float_to_c_short(button_value)
                elif input_name == 'RIGHT_STICK_X':
                    xbox_report.sThumbRX = float_to_c_short(button_value)
                elif input_name == 'RIGHT_STICK_Y':
                    xbox_report.sThumbRY = float_to_c_short(button_value)
                elif input_name == 'LEFT_TRIGGER':
                    xbox_report.bLeftTrigger = byte_to_c_byte(button_value)
                elif input_name == 'RIGHT_TRIGGER':
                    xbox_report.bRightTrigger = byte_to_c_byte(button_value)
        ...
        # Axis Value
            ...
            button_value = gamepad.get_axis(idx)
            sameSign = have_same_sign(button_value, val)
            absVal = max((abs(button_value) - 1), 0)

            if absVal and sameSign:
                # Assign value to the proper XBOX report field
                if input_name == 'LEFT_STICK_X':
                    xbox_report.sThumbLX = float_to_c_short(button_value)
                elif input_name == 'LEFT_STICK_Y':
                    xbox_report.sThumbLY = float_to_c_short(button_value * -1)
                elif input_name == 'RIGHT_STICK_X':
                    xbox_report.sThumbRX = float_to_c_short(button_value)
                elif input_name == 'RIGHT_STICK_Y':
                    xbox_report.sThumbRY = float_to_c_short(button_value * -1)
                elif input_name == 'LEFT_TRIGGER':
                    xbox_report.bLeftTrigger = float_to_c_byte(button_value)
                elif input_name == 'RIGHT_TRIGGER':
                    xbox_report.bRightTrigger = float_to_c_byte(button_value)
        # Standard Button
        if typ == 1:
            button_value = gamepad.get_button(idx)
            if input_name == 'LEFT_STICK_X':
                ...
                xbox_report.bLeftTrigger = byte_to_c_byte(button_value * 255)
            elif input_name == 'RIGHT_TRIGGER':
                xbox_report.bRightTrigger = byte_to_c_byte(button_value * 255)
            elif button_value:
                # Return corresponding XBOX_BUTTON value based on input name
                xbox_report.wButtons += getattr(XBOX_BUTTON, 'XBOX_' + input_name)
...
def wait_for_no_pygamepad_input(pygame, gamepad):
    ...
    while awaiting_silence:
        ...
        for i in range(gamepad.get_numaxes()):
            ...
            axis_value += abs(read_val) if abs(read_val - avg_baseline[i]) > 0.5 else 0
            # axis_value += read_val if abs(read_val) - 1 > .30 else 0
...
def get_pygamepad_input_old(pygame, gamepad):
    BUTTON = 1
    STICK = 2
    DPAD = 4
    while True:
        ...
        for i in range(input_info[0]):  # gamepad.get_numaxes()):
            ...
            if abs(value - avg_baseline[i]) > .5:
                print(f'Axis:{i}::{gamepad.get_axis(i)} - {avg_baseline[i]}')
                return STICK, i, 1 if value > 0 else -1
...
def get_pygamepad_input(pygame, gamepad):
    ...
    while True:
        pygame.event.pump()  # This replaces pygame.event.get() for efficiency

        # Allow for a user abort on input
        if keyboard.is_pressed('esc'):
            while keyboard.is_pressed('esc'):
                pygame.time.delay(10)  # Replace with a short delay to mimic Sleep(10)
            return input

        # Iterate over all gamepad axes
        for i in range(input_info[0]):  # gamepad.get_numaxes()):
            axis_value = gamepad.get_axis(i)
            if abs(axis_value - avg_baseline[i]) > 0.75:  # Using threshold directly
                return STICK, i, -1 if axis_value < 0 else 1
        ...
        for i in range(input_info[2]):  # gamepad.get_numhats()):
            hat_direction = get_hat_direction_from_tuple(gamepad.get_hat(i))
            if hat_direction:
                return DPAD, i, hat_direction
...
def set_pygame_mapping(pygame, gamepad, buttons: PyGameButtonMapping, input_list: [str]):
    ...
    input_info, baseline_reports = get_pygamepad_baseline(pygame, gamepad, 16)
    num_inputs = input_info[3]
    avg_baseline, median_baseline, mode_baseline, range_baseline = baseline_reports

    #print(get_bytes_from_report(parse_index_string(f'0-{num_inputs - 1}'), avg_baseline))
...
def pygame_rumble(gamepad, left_motor, right_motor, duration=200):
    return gamepad.rumble(left_motor * RUMBLE_CONVERSION, right_motor * RUMBLE_CONVERSION, duration)
